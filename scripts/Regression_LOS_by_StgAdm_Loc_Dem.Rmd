...
ut <- readRDS("/Users/sreynolds2/Documents/GitHub/MS-UTSW_Covid_Staging/data/ut_df_unique_pts.rds") %>% 
  select(ID, age, sex, race, ethnicity, LOS, max_stage, stage_adm)
...
# Since max_stage and stage_adm are coded as ordinal vars for UT, need to do the same for UC 
uc <- uc %>%
  mutate(max_stage = factor(max_stage, ordered = TRUE, levels = c("4", "5", "6", "7", "8", "9", "10")), 
         stage_adm = factor(stage_adm, ordered = TRUE, levels = c("4", "5", "6", "7", "8", "9")))
...
# Log-transform LOS and assign to `t.LOS` and ungroup data
regdf <- regdf %>% 
  mutate(t.LOS = log(LOS)) %>% 
  ungroup()
...
library(fastDummies)
#df <- regdf %>% dummy_cols(select_columns = race) %>% dummy_cols(select_columns = age)
#responses = as.matrix(regdf$LOS)
#predictors = as.matrix(regdf$age, regdf$race, regdf$stage_adm)
