---
title: "Length of Stay (LOS) Regression Analysis"
...
1.  Prepares the cleaned UTSW and UCSF datasets for analysis

    -   Recode levels for `stage_adm` - see below for details

    -   Transforms outcome variable `LOS` on log scale

    -   Merges UTSW and UCSF dataset and select only relevant variables for analysis
...
5.  Fit linear models with and w/o interaction term between `stage_adm` and `age` and compare output
...
# Create vector of predictor names
    # For vars with >2 categories (race, ethnicity), we can remove one of the columns
pred.names <- c("age", race_names[-1], "stage_adm", ethnicity_names[-1], "loc_UTSW", "sex_Male")
pred.names
...
**Interpretation:** Used polspline::polymars(prdictors, responses) to identify which parameters were most influential. It confirmed that we should include stage at admission (`stage_adm`), age (`age`), and location (`loc`) as predictors, and should consider including age as a non-linear - perhaps quadratic - variable in our model, given the knot at age 73. It also suggested that there is an interaction between age and stage at admission.
...
m0 <- lm(t.LOS ~ stage_adm + loc + age, data = regdf)
summary(m0)

# Model with age as polynomial of 2nd degree (quadratic)
m1 <- lm(t.LOS ~ stage_adm + loc + poly(age,2), data = regdf)
summary(m1)

# Model with age as smooth term (semi-parametric)
m2 <- mgcv::gam(t.LOS ~ stage_adm + loc + s(age), data = regdf)
summary(m2)
...
m3 <- lm(t.LOS ~ stage_adm + loc + poly(age,2) + stage_adm:age, data = regdf)
summary(m3)

# BMA::bic.glm 
summary(BMA::bic.glm(t.LOS ~ stage_adm + loc + poly(age,2) + stage_adm:age, data = regdf, glm.family = "gaussian"))
...
# Stage at admission
m.stage_adm <- lm(t.LOS ~ stage_adm, data = regdf)
summary(m.stage_adm)
...
m.interx <- lm(t.LOS ~ stage_adm:age, data = regdf)
summary(m.interx)

# Interaction between stage at admission and age as quadratic 
m.interx.polyage <- lm(t.LOS ~ stage_adm:poly(age,2), data = regdf)
